...
export const UserCard = ({ u }) => {
  ...
  return (
    <>
      <article className="flex items-center justify-between bg-white border border-gray-200 px-4 py-3">
        ...
        <button
          ...
          className={`cursor-pointer px-4 py-1.5 text-xs sm:text-base font-medium bg-blue-600 text-white hover:bg-blue-700 transition ${
            user.id === u.id && "hidden"
          }`}
        >
          {userFollowing &&
        ...
        <Link
          to={`/profile/${user.id}`}
          className={`cursor-pointer px-4 py-1.5 text-xs sm:text-base font-medium bg-gray-400 text-white hover:bg-gray-600 transition ${
            user.id !== u.id && "hidden"
          }`}
        >
          Ver Perfil
